...
func pr(string):
	print('[CONFIG] ', string)
func CheckBools(string : String):
	var lowertxt = string.to_lower()
	if lowertxt == 'true' or lowertxt == 'false':
		return str2var(lowertxt)
	return string
		...
		if x_button.pressed == true:
			...
			for config_nodes in range(CONFIG_LIST.get_child_count()):
				...
				for config_values in range(node.Config_Values.size()):
					...
					var Config_Value = CheckBools(Config_Setting.config_value)
